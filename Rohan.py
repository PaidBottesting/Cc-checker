...
API_ENDPOINT = "https://xchecker.cc/api.php"
...
async def check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            response = requests.get(f"{API_ENDPOINT}?cc={card_details}", timeout=10)
            elapsed_time = time.time() - start_time
            logger.info(f"xchecker.cc API response: status={response.status_code}, text={response.text}")
                ...
                try:
                    ...
                    if data.get("status") == "Live":
                        await update.message.reply_text(
                            ...
                            f"🌐 *Gateway* ⇾ xchecker.cc\n"
                            f"📋 *Response* ⇾ {escape_markdown(data.get('details', 'Approved'))}\n"
                        ...
                        error = data.get("details", data.get("error", "Invalid card"))
                        await update.message.reply_text(
                            f"❌ *Declined* ❌\n\n"
                            f"💳 *CC* ⇾ `{escape_markdown(card_details)}`\n"
                            f"🌐 *Gateway* ⇾ xchecker.cc\n"
