        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if data.get("valid"):
                        card_type = f"{data.get('card', 'UNKNOWN')} - {data.get('type', 'UNKNOWN')} - {data.get('level', 'UNKNOWN')}"
                        bank = data.get("bank", "UNKNOWN")
                        country = f"{data.get('country', 'UNKNOWN')} ({data.get('countrycode', '')})"
                        status_text = "Charge OK." if is_live else "Invalid card"
                        result = "Approved" if is_live else "Declined"
                        await update.message.reply_text(
                            f"{'✅' if is_live else '❌'} *{result}* {'✅' if is_live else '❌'}\n\n"
                            f"💳 *CC* ⇾ `{escape_markdown(card_details)}`\n"
                            f"🌐 *Gateway* ⇾ Bincodes\.com\n"
                            f"📋 *Response* ⇾ {escape_markdown(status_text)}\n"
                        ...
                        error = data.get("error", "Invalid card number")
                        await update.message.reply_text(
                            f"❌ *Declined* ❌\n\n"
                            f"💳 *CC* ⇾ `{escape_markdown(card_details)}`\n"
                            f"🌐 *Gateway* ⇾ Bincodes\.com\n"
            ...
            if response.status_code == 200:
                try:
                    ...
                    if data.get("valid"):
                        ...
                        bank = data.get("bank", "UNKNOWN")
                        country = f"{data.get('country', 'UNKNOWN')} ({data.get('countrycode', '')})"
                        status_text = "Charge OK." if is_live else "3DS Authentication Rejected"
                        result = "Approved" if is_live else "Rejected"
                        await update.message.reply_text(
                            f"{'✅' if is_live else '❌'} *{result}* {'✅' if is_live else '❌'}\n\n"
                            f"💳 *CC* ⇾ `{escape_markdown(card_details)}`\n"
