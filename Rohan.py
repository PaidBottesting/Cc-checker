...
API_ENDPOINT = "https://api.bincodes.com/cc/"
    ...
    try:
        ...
        try:
            ...
            response = requests.get(
                f"{API_ENDPOINT}?format=json&api_key={BINCODES_API_KEY}&cc={card_number[:8]}",
                timeout=10
            )
            elapsed_time = time.time() - start_time
            logger.info(f"Bincodes API response: status={response.status_code}, text={response.text}")
            # Mock validation since Bincodes doesn't check expiry/CVC
            is_live = card_number == "4242424242424242"
            if response.status_code == 200:
                try:
                    data = response.json()
                    logger.debug(f"Parsed API data: {data}")
                    if data.get("valid"):
                        card_type = escape_markdown(f"{data.get('card', 'UNKNOWN')} - {data.get('type', 'UNKNOWN')} - {data.get('level', 'UNKNOWN')}")
                        bank = escape_markdown(data.get("bank", "UNKNOWN"))
                        country = escape_markdown(f"{data.get('country', 'UNKNOWN')} ({data.get('countrycode', '')})")
                        status_text = escape_markdown("Charge OK." if is_live else "Invalid card")
                        result = escape_markdown("Approved" if is_live else "Declined")
                        message = (
                            f"{'✅' if is_live else '❌'} *{result}* {'✅' if is_live else '❌'}\n\n"
                            f"💳 *CC* ⇾ `{escape_markdown(card_details)}`\n"
                            f"🌐 *Gateway* ⇾ Bincodes\.com\n"
                            f"📋 *Response* ⇾ {status_text}\n"
                            f"🔍 *BIN Info*: {card_type}\n"
                            f"🏦 *Bank*: {bank}\n"
                            f"🌍 *Country*: {country}\n"
                            f"⏱️ *Took* ⇾ {elapsed_time:.2f} seconds"
                        )
                        logger.debug(f"Sending message: {message}")
                        await update.message.reply_text(message, parse_mode="MarkdownV2")
                    else:
                        error = escape_markdown(data.get("error", "Invalid card number"))
                        message = (
                            f"❌ *Declined* ❌\n\n"
                            f"💳 *CC* ⇾ `{escape_markdown(card_details)}`\n"
                            f"🌐 *Gateway* ⇾ Bincodes\.com\n"
                ...
                message = f"❌ *Error*: API returned status {response.status_code}\."
                logger.debug(f"Sending message: {message}")
                await update.message.reply_text(message, parse_mode="MarkdownV2")
        except requests.exceptions.Timeout:
            message = (
                f"🚨 *Connection Timeout* 🚨\n"
                f"💬 *Details*: API request timed out after 10 seconds\."
            )
            logger.debug(f"Sending message: {message}")
            await update.message.reply_text(message, parse_mode="MarkdownV2")
            ...
            if response.status_code == 200:
                try:
                    ...
                    if data.get("valid"):
                        ...
                        bank = escape_markdown(data.get("bank", "UNKNOWN"))
                        country = escape_markdown(f"{data.get('country', 'UNKNOWN')} ({data.get('countrycode', '')})")
                        status_text = escape_markdown("Charge OK." if is_live else "3DS Authentication Rejected")
                        result = escape_markdown("Approved" if is_live else "Rejected")
                        message = (
                            f"{'✅' if is_live else '❌'} *{result}* {'✅' if is_live else '❌'}\n\n"
                            f"💳 *CC* ⇾ `{escape_markdown(card_details)}`\n"
                            f"🌐 *Gateway* ⇾ Bincodes\.com 3DS\n"
                            f"📋 *Response* ⇾ {status_text}\n"
                            f"🔍 *Info*: {card_type}\n"
                            f"🏦 *Issuer*: {bank}\n"
                            f"🌍 *Country*: {country}\n"
...
def main():
    ...
    if not BINCODES_API_KEY:
        logger.error("BINCODES_API_KEY missing in .env")
        return
