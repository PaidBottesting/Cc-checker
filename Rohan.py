...
def load_admins():
    if os.path.exists(ADMIN_FILE):
        with open(ADMIN_FILE, "r") as f:
            return json.load(f)
    return [123456789]  # Default admin ID (replace with yours)
...
async def addadmin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if new_admin in ADMIN_IDS:
            await update.message.reply_text(f"⚠️ *Error*: User {escape_markdown(new_admin)} is already an admin.", parse_mode="MarkdownV2")
            return

        ADMIN_IDS.append(new_admin)
        save_admins(ADMIN_IDS)
        await update.message.reply_text(f"✅ *Success*: User {escape_markdown(new_admin)} added as admin.", parse_mode="MarkdownV2")
    except ValueError:
...
async def removeadmin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if admin_id not in ADMIN_IDS:
            await update.message.reply_text(f"⚠️ *Error*: User {escape_markdown(admin_id)} is not an admin.", parse_mode="MarkdownV2")
            return

        ADMIN_IDS.remove(admin_id)
        save_admins(ADMIN_IDS)
        await update.message.reply_text(f"✅ *Success*: User {escape_markdown(admin_id)} removed from admins.", parse_mode="MarkdownV2")
    except ValueError:
        await update.message.reply_text("❌ *Error*: Invalid user ID.", parse_mode="MarkdownV2")
